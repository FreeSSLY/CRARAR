...
const Index = () => {
  ...
  const [activeBuscaTab, setActiveBuscaTab] = useState("todos");
  ...
  const animaisCrarar = animaisFormatted.filter((animal) => {
    const tutor = tutores.find((t) => t.id === animal.tutor_id);
    return tutor?.nome.toLowerCase() === "crarar";
  });

  const outrosAnimais = animaisFormatted.filter((animal) => {
    const tutor = tutores.find((t) => t.id === animal.tutor_id);
    return tutor?.nome.toLowerCase() !== "crarar";
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-crarar-secondary to-white">
      <div className="container mx-auto p-4 sm:p-6">
        ...
        <Tabs
          value={activeTab}
          onValueChange={(tab) => !isSearching && setActiveTab(tab)}
          className="w-full"
        >
          <TabsList className="grid w-full grid-cols-2 mb-4 sm:mb-6 bg-white shadow-sm h-auto">
            <TabsTrigger
              value="cadastro"
              className="data-[state=active]:bg-crarar-primary data-[state=active]:text-white transition-all duration-300 flex flex-col sm:flex-row items-center gap-1 sm:gap-2 py-2 sm:py-1.5 px-2 sm:px-3 text-xs sm:text-sm mobile-button"
            >
          ...
          <TabsContent value="busca" className="animate-slide-in">
            <Tabs
              value={activeBuscaTab}
              onValueChange={setActiveBuscaTab}
              className="w-full"
            >
              <TabsList className="grid w-full grid-cols-2 mb-4 sm:mb-6 bg-white shadow-sm h-auto">
                <TabsTrigger
              ...
              <TabsContent value="crarar">
                <SearchResults
                  tutores={tutores}
                  animais={animaisCrarar}
                  atendimentos={atendimentosFormatted}
                  onUpdateTutores={handleUpdateTutores}
                  onUpdateAnimais={handleUpdateAnimais}
                  onUpdateAtendimentos={handleUpdateAtendimentos}
