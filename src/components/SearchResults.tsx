...
const SearchResults = ({
  ...
  const [changingTutorAnimal, setChangingTutorAnimal] = useState<Animal | null>(
    null
  );
  ...
  useEffect(() => {
    if (onSearchingChange) {
      const isAnyModalOpen =
        ...
        editingAtendimento ||
        changingTutorAnimal;
      onSearchingChange(!!isAnyModalOpen);
    }
  }, [
    selectedRecord,
    editingAnimal,
  ...
  const animaisOrdenados = [...animais].sort(
    (a, b) =>
  ...
  const filteredAnimais = useMemo(() => {
    ...
    return animaisOrdenados.filter((animal) => {
      ...
      const especieMatch = animal.especie.toLowerCase().includes(searchLower);
      const racaMatch = animal.raca.toLowerCase().includes(searchLower);

      // Buscar por nome do tutor
      const nomeTutorMatch = (tutor?.nome || "")
        .toLowerCase()
        .includes(searchLower);

      return nomeAnimalMatch || especieMatch || racaMatch || nomeTutorMatch;
    });
  ...
  //       cpf: editingTutor.cpf === "" ? null : editingTutor.cpf,
  ...
  const handleChangeTutor = (animal: Animal) => {
    if (readOnly) return;
    setChangingTutorAnimal(animal);
  };

  const handleSaveNewTutor = async (newTutorId: string) => {
    if (!changingTutorAnimal || readOnly || !onUpdateAnimal) return;

    try {
      await onUpdateAnimal(changingTutorAnimal.id, { tutor_id: newTutorId });
      setChangingTutorAnimal(null);
      toast({
        title: "Sucesso!",
        description: "Tutor do animal atualizado com sucesso.",
      });
    } catch (error) {
      console.error("Error updating animal's tutor:", error);
      toast({
        title: "Erro",
        description: "Erro ao atualizar tutor do animal. Tente novamente.",
        variant: "destructive",
      });
    }
  };
  ...
  const generatePDF = (
    animal: Animal,
    tutor: Tutor,
    atendimentosAnimal: Atendimento[]
  ) => {
    const doc = new jsPDF();
    ...
    doc.text(
      "CRARAR - Sistema de Gestão Veterinária",
      pageWidth / 2,
      yPosition,
      { align: "center" }
    );
    ...
    doc.text("Relatório Detalhado do Animal", pageWidth / 2, yPosition, {
      align: "center",
    });
    ...
    animalInfo.forEach((info, index) => {
      ...
      if (index % 2 !== 0 || index === animalInfo.length - 1) {
        yPosition += 6;
      }
    });
    yPosition += 10;
    ...
    doc.setTextColor(59, 122, 87);
    doc.text(
      `HISTÓRICO DE ATENDIMENTOS (${atendimentosAnimal.length})`,
      margin,
      yPosition
    );
    yPosition += 10;
    ...
    if (atendimentosAnimal.length > 0) {
      atendimentosAnimal.forEach((atendimento, index) => {
        ...
        doc.setTextColor(59, 122, 87);
        doc.text(
          `Atendimento ${index + 1} - ${formatDate(atendimento.data)}`,
          margin,
          yPosition
        );
        yPosition += 8;
        ...
        if (atendimento.proximo_retorno) {
          atendimentoInfo.push(
            `Próximo retorno: ${formatDate(atendimento.proximo_retorno)}`
          );
        }
    ...
    doc.save(`Relatorio_${animal.nome}_${getLocalDateString(now)}.pdf`);
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            <div className="flex-1">
              <Input
                placeholder="Nome do animal ou nome do tutor..."
                value={searchTerm}
                onChange={(e) => {
                  setSearchTerm(e.target.value);
                  if (onSearchingChange) {
                    onSearchingChange(e.target.value !== "");
                  }
                }}
      ...
      <div className="space-y-3 sm:space-y-4">
        {filteredAnimais.map((animal) => {
          ...
          return (
            <Card
              key={animal.id}
              className="animate-fade-in shadow-sm hover:shadow-md transition-shadow"
            >
                    ...
                    <div className="space-y-1 text-xs sm:text-sm text-muted-foreground">
                      ...
                      <div className="flex flex-wrap gap-2 sm:gap-4 text-xs">
                        ...
                        <span>
                          ...
                          {atendimentosAnimal.length}
                        </span>
                      </div>
                      {ultimoAtendimento && (
                        <p className="text-xs">
                          <strong>Último atendimento:</strong>{" "}
                          {formatDate(ultimoAtendimento.data)}
                        </p>
                      )}
                    </div>
                      ...
                      <DialogContent className="w-[95vw] max-w-4xl max-h-[85vh] overflow-y-auto">
                        <DialogHeader className="pb-4">
                          <DialogTitle className="flex items-center gap-2 text-lg sm:text-xl">
                            {getEspecieIcon(animal.especie)} Detalhes Completos
                            - {animal.nome}
                          </DialogTitle>
                          <DialogDescription className="text-sm">
                            Informações do tutor, animal e histórico completo de
                            atendimentos
                          </DialogDescription>
                        </DialogHeader>
                        ...
                        {selectedRecord && (
                          <div className="space-y-4 sm:space-y-6">
                            <Card>
                              <CardHeader className="pb-3">
                                <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                                  {getEspecieIcon(selectedRecord.data.especie)}{" "}
                                  Dados do Animal
                                </CardTitle>
                              </CardHeader>
                              <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                                <div className="space-y-2">
                                  <p>
                                    <strong>Nome:</strong>{" "}
                                    {selectedRecord.data.nome}
                                  </p>
                                  <p>
                                    <strong>Espécie:</strong>{" "}
                                    {selectedRecord.data.especie}
                                  </p>
                                  <p>
                                    <strong>Raça:</strong>{" "}
                                    {selectedRecord.data.raca}
                                  </p>
                                  <p>
                                    <strong>Sexo:</strong>{" "}
                                    {selectedRecord.data.sexo}
                                  </p>
                                </div>
                                <div className="space-y-2">
                                  <p>
                                    <strong>Cor:</strong>{" "}
                                    {selectedRecord.data.cor}
                                  </p>
                                  <p>
                                    <strong>Peso:</strong>{" "}
                                    {selectedRecord.data.peso} kg
                                  </p>
                                  <p>
                                    <strong>Idade:</strong>{" "}
                                    {calcularIdade(
                                      selectedRecord.data.data_nascimento
                                    )}
                                  </p>
                                  <p>
                                    <strong>Data de Adesão:</strong>{" "}
                                    {formatDate(
                                      selectedRecord.data.data_adesao
                                    )}
                                  </p>
                                </div>
                              </CardContent>
                              ...
                              <Card>
                                ...
                                <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                                  <div className="space-y-2">
                                    <p>
                                      <strong>Nome:</strong>{" "}
                                      {selectedRecord.tutor.nome}
                                    </p>
                                    <p>
                                      <strong>CPF:</strong>{" "}
                                      {selectedRecord.tutor.cpf}
                                    </p>
                                    <p>
                                      <strong>Telefone:</strong>{" "}
                                      {selectedRecord.tutor.telefone}
                                    </p>
                                  </div>
                                  <div className="space-y-2">
                            ...
                            <Card>
                              <CardHeader className="pb-3">
                                <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                                  <Heart className="w-4 h-4" /> Histórico de
                                  Atendimentos (
                                  {selectedRecord.atendimentosAnimal?.length ||
                                    0}
                                  )
                                </CardTitle>
                              </CardHeader>
                              <CardContent>
                                ...
                                0 ? (
                                  <div className="space-y-3 max-h-48 sm:max-h-60 overflow-y-auto">
                                    {selectedRecord.atendimentosAnimal.map(
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  generatePDF(
                                    selectedRecord.data,
                                    selectedRecord.tutor,
                                    selectedRecord.atendimentosAnimal
                                  )
                                }
                                className="text-sm"
                              >
                              ...
                              {canEdit && (
                                <div className="flex flex-col sm:flex-row gap-2">
                                  <Button
                                    variant="outline"
                                    onClick={() =>
                                      handleEditAnimal(selectedRecord.data)
                                    }
                                    className="text-sm"
                                  >
                                  ...
                                  <Button
                                    variant="outline"
                                    onClick={() =>
                                      handleChangeTutor(selectedRecord.data)
                                    }
                                    className="text-sm"
                                  >
                                    <Edit className="w-4 h-4 mr-2" />
                                    ...
                                    <AlertDialog>
                                      ...
                                      <AlertDialogContent className="w-[95vw] max-w-md">
                                        <AlertDialogHeader>
                                          ...
                                          <AlertDialogDescription className="text-sm">
                                            ...
                                            <span className="text-red-600 font-semibold">
                                              Esta ação irá remover também todos
                                              os atendimentos relacionados a
                                              este animal!
                                            </span>
                                            <br />
                                            <br />
                                        ...
                                        <AlertDialogFooter className="flex-col sm:flex-row gap-2">
                                          ...
                                          <AlertDialogAction
                                            onClick={() =>
                                              handleDelete(selectedRecord.data)
                                            }
                                            className="bg-red-600 hover:bg-red-700 text-sm"
                                          >
            ...
            <form onSubmit={handleSaveAnimal} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    value={editingAnimal.especie}
                    onValueChange={(value) =>
                      setEditingAnimal({ ...editingAnimal, especie: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  ...
                  <Select
                    value={editingAnimal.sexo}
                    onValueChange={(value) =>
                      setEditingAnimal({ ...editingAnimal, sexo: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
      ...
      {/* Modal de Mudança de Tutor */}
      {changingTutorAnimal && !readOnly && (
        <Dialog
          open={!!changingTutorAnimal}
          onOpenChange={() => setChangingTutorAnimal(null)}
        >
          <DialogContent className="w-[95vw] max-w-md">
            <DialogHeader>
              <DialogTitle>
                Mudar Tutor do Animal - {changingTutorAnimal.nome}
              </DialogTitle>
              <DialogDescription>
                Selecione o novo tutor para {changingTutorAnimal.nome}.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <Label htmlFor="tutor-select">Novo Tutor</Label>
              <ComboBoxResponsive
                options={tutores.map((tutor) => ({
                  label: `${tutor.nome} - ${tutor.telefone}`,
                  value: tutor.id,
                }))}
                value={changingTutorAnimal.tutor_id}
                onChange={(newTutorId) =>
                  setChangingTutorAnimal({
                    ...changingTutorAnimal,
                    tutor_id: newTutorId,
                  })
                }
                placeholder="Selecione o novo tutor"
              ...
              <div className="flex justify-end gap-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setChangingTutorAnimal(null)}
                >
                  Cancelar
                </Button>
                <Button
                  type="button"
                  onClick={() =>
                    handleSaveNewTutor(changingTutorAnimal.tutor_id)
                  }
                >
                  Salvar
                </Button>
      ...
      {editingAtendimento && !readOnly && (
        <Dialog
          ...
          onOpenChange={() => setEditingAtendimento(null)}
        >
          <DialogContent className="w-[95vw] max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Editar Atendimento - {formatDate(editingAtendimento.data)}
              </DialogTitle>
            </DialogHeader>
