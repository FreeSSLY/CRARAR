...
const AtendimentoForm = ({
  ...
  const animalFilter = (value: string, search: string) => {
    const lowerCaseSearch = search.toLowerCase();
    const animalName = value.split(" (")[0]; // Extract animal name before '(' for species

    if (animalName.toLowerCase().startsWith(lowerCaseSearch)) {
      return 1;
    }
    return 0;
  };
  ...
  return (
    <Card className="animate-fade-in">
      <CardHeader>
        ...
        <CardDescription>Documente a consulta veterinária</CardDescription>
      </CardHeader>
      <CardContent>
        {animais.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-muted-foreground mb-4">
              Você precisa cadastrar um animal primeiro para registrar um
              atendimento.
            </p>
            <Button
              variant="outline"
              onClick={() =>
                toast({
                  title: "Dica",
                  description:
                    "Vá para a aba 'Animal' e cadastre um pet primeiro.",
                })
              }
            >
                  ...
                  <FormItem className="flex flex-col">
                    ...
                    <ComboBoxResponsive
                      options={animais.map((animal) => ({
                        label: `${animal.nome} (${animal.especie}) - Tutor: ${
                          getTutorByAnimal(animal.id)?.nome
                        }`,
                        value: animal.id,
                      }))}
                      value={field.value}
                      ...
                      emptyPlaceholder="Nenhum animal encontrado."
                      filter={animalFilter}
                    />
                    <FormMessage />
                  </FormItem>
              ...
              {selectedAnimal && (
                <div className="bg-crarar-secondary p-3 rounded-lg">
                  ...
                  <p className="text-sm text-crarar-text">
                    {selectedAnimal.nome} ({selectedAnimal.especie}) - Tutor:{" "}
                    {getTutorByAnimal(selectedAnimal.id)?.nome}
                  </p>
                </div>
              )}
